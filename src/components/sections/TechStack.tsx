...
function TechCard({ tech, index }: { tech: typeof techs[0]; index: number }) {
  ...
  return (
    <motion.div
      ...
      className="flex flex-col items-center gap-3 p-6 rounded-xl bg-white/[0.03] border border-white/5 hover:border-purple-500/20 hover:bg-white/[0.06] transition-all duration-300 group"
    >
      ...
      <span className="text-sm text-zinc-400 group-hover:text-zinc-200 transition-colors">{tech.name}</span>
    </motion.div>
  );
}
    ...
    <section id="tech-stack" className="relative py-32">
      ...
      <div className="container-cinematic section-padding relative z-10">
        <motion.div
          ...
          className="mb-4"
        >
          <span className="text-xs tracking-[0.2em] uppercase text-purple-400">Tech Stack</span>
        </motion.div>
